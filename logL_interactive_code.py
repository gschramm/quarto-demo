...
# ---------------------------------------------------------------------------
try:
    _p = {k: params[k] for k in (   # pyodide: OJS reactive proxy
        "y1", "y2", "b1", "b2",
        "xmin", "xmax",
        "x1_start", "x2_start",
    )}
    ...
    _parser.add_argument("--y1",       type=int,   default=1,    metavar="N", help="measured counts bin 1 (default: 1)")
    _parser.add_argument("--y2",       type=int,   default=2,    metavar="N", help="measured counts bin 2 (default: 2)")
    _parser.add_argument("--b1",       type=float, default=0.5,  metavar="F", help="background bin 1 (default: 0.5)")
    _parser.add_argument("--b2",       type=float, default=0.5,  metavar="F", help="background bin 2 (default: 0.5)")
    _parser.add_argument("--xmin",     type=float, default=-0.5, metavar="F", help="axis min for x1 and x2 (default: -0.5)")
    _parser.add_argument("--xmax",     type=float, default=5.0,  metavar="F", help="axis max for x1 and x2 (default: 5.0)")
    _parser.add_argument("--lmin",     type=float, default=None, metavar="F", help="colorbar min (default: auto)")
    _parser.add_argument("--lmax",     type=float, default=None, metavar="F", help="colorbar max (default: auto)")
    _parser.add_argument("--x1_start", type=float, default=2.5,  metavar="F", help="MLEM start x1 (default: 2.5)")
    _parser.add_argument("--x2_start", type=float, default=2.5,  metavar="F", help="MLEM start x2 (default: 2.5)")
    _args = _parser.parse_args()
    _p    = {k: getattr(_args, k) for k in ("y1", "y2", "b1", "b2", "xmin", "xmax", "x1_start", "x2_start")}
    _lmin = _args.lmin
    _lmax = _args.lmax
...
# ---------------------------------------------------------------------------
fig, ax = plt.subplots(3, 1, figsize=(4, 8), layout="constrained")
ax_map, ax_L, ax_dist = ax

# — Contour map —
cs = ax_map.contourf(X1g, X2g, Z_plot, levels=levels, cmap="magma", alpha=0.85,
                     vmin=lmin, vmax=lmax)
ax_map.axhline(0, color="black", lw=1.0)
ax_map.axvline(0, color="black", lw=1.0)
ax_map.plot(tx,    ty,    "k.-", ms=5,  zorder=5, label="MLEM")
ax_map.plot(tx3,   ty3,   "c.-", ms=5,  zorder=5, label=rf"MLEM-3 ($\gamma$={gamma:.2f})")
ax_map.plot(tx[0], ty[0], "ks",  ms=7,  zorder=6, label="start")
ax_map.plot(xhat1, xhat2, "b^",  ms=9,  zorder=6, label=r"$\hat{\mathbf{x}}$")
ax_map.plot(xml1,  xml2,  "r*",  ms=11, zorder=6, label="unconstrained max")
cb = plt.colorbar(cs, ax=ax_map, label=r"$\mathcal{L}(x_1,x_2)$",
                  fraction=0.046, pad=0.04)
cb.ax.yaxis.set_major_formatter(FormatStrFormatter("%.1f"))
ax_map.set_xlabel(r"$x_1$");  ax_map.set_ylabel(r"$x_2$")
ax_map.set_xlim(xmin, xmax);  ax_map.set_ylim(xmin, xmax)
ax_map.set_aspect("equal", adjustable="box")
ax_map.set_title(
    rf"$y_1={y1},\; y_2={y2},\; b_1={b1:.1f},\; b_2={b2:.1f}$", fontsize="medium"
)
...
ax_L.axhline(logL_scalar(xhat1, xhat2), color="b", lw=1, ls="--",
             label=r"$\mathcal{L}(\hat{x})$")
...
# — ||x − x̂|| vs iteration —
ax_dist.plot(iters, d_mlem,  "k.-", ms=4, label="MLEM")
ax_dist.plot(iters, d_mlem3, "c.-", ms=4, label="MLEM-3")
